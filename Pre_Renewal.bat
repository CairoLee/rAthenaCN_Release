...
:startLogin
call :getLoginStatus
if "%login_running%" == "false" ( start cmd /k tools\batches\logserv-pre.bat %restart_mode%
) else echo "Login serv is already running pid=%LoginServPID%" 
goto :EOF

:startChar
call :getCharStatus
if "%char_running%" == "false" ( start cmd /k tools\batches\charserv-pre.bat %restart_mode%
) else echo "Char serv is already running, pid=%CharServPID%" 
goto :EOF

:startMap
call :getMapStatus
if "%map_running%" == "false" ( start cmd /k tools\batches\mapserv-pre.bat %restart_mode%
...
:getLoginStatus
for /F "TOKENS=1,2,*" %%a in ('tasklist /FI "IMAGENAME eq login-server-pre.exe"') do set LoginServPID=%%b
echo(%LoginServPID%|findstr "^[-][1-9][0-9]*$ ^[1-9][0-9]*$ ^0$">nul&& set "login_running=true" || set "login_running=false"
goto :EOF

:getCharStatus
for /F "TOKENS=1,2,*" %%a in ('tasklist /FI "IMAGENAME eq char-server-pre.exe"') do set CharServPID=%%b
echo(%CharServPID%|findstr "^[-][1-9][0-9]*$ ^[1-9][0-9]*$ ^0$">nul&& set "char_running=true" || set "char_running=false"
goto :EOF

:getMapStatus
for /F "TOKENS=1,2,*" %%a in ('tasklist /FI "IMAGENAME eq map-server-pre.exe"') do set MapServPID=%%b
